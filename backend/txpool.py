...
class TxPool:
    ...
    def ordered_all(self):
        ...
        The display (and packaging-preview) order is the arrival order:
        earlier-submitted transactions come first, exactly as tracked by
        ``self._order``.  Transactions must *not* be sorted by ``txid`` —
        txids are content hashes, so such a sort produces a deterministic
        but apparently shuffled order unrelated to submission time.
        """
        return self.all()
